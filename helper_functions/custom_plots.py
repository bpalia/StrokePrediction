...
def hist_box_plot(
    df: pd.DataFrame, x: str, title: str, hue: str = None, xlabel: str = None
) -> Figure:
    """Function to plot boxplot above histogram."""
    fig, (ax_box, ax_hist) = plt.subplots(
        2, sharex=True, gridspec_kw={"height_ratios": (0.15, 0.85)}
    )
    sns.boxplot(df, x=x, y=hue, ax=ax_box)
    ax_box.set_xlabel("")
    sns.histplot(df, x=x, hue=hue, ax=ax_hist)
    if xlabel is not None:
